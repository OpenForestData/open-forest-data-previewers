...
var IIP = new Class({
  ...
  parseMetaData: function (response) {
    ...
    var max_size = { w: parseInt(size[0].substring(1, size[0].length)), h: parseInt(size[1]) };
    tmp = response.split('Tile-size');
    size = tmp[1].split(' ');
    var tileSize = { w: parseInt(size[0].substring(1, size[0].length)), h: parseInt(size[1]) };
