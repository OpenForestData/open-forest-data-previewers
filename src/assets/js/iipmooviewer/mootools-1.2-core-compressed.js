      ...
      switch (ctype) {
        case 'function':
          if (!arguments.callee.caller) {
            self[key] = eval(
              '(' +
                String(current).replace(/\bthis\.parent\(\s*(\))?/g, function (full, close) {
...
Element.implement({
  ...
  getSelected: function () {
    return new Elements(
      $A(this.options).filter(function (A) {
        return A.selected;
      })
    );
  },
    ...
    this.getElements('input, select, textarea').each(function (B) {
      ...
      var C =
        B.tagName.toLowerCase() == 'select'
          ? Element.getSelected(B).map(function (D) {
              return D.value;
            })
          : (B.type == 'radio' || B.type == 'checkbox') && !B.checked
          ? null
          : B.value;
...
Element.Properties.href = {
  get: function () {
    return !this.href
      ? null
      : this.href.replace(new RegExp('^' + document.location.protocol + '//' + document.location.host), '');
  },
};
...
Selectors.Utils = {
  ...
  search: function (J, I, O) {
    var B = [];
    var C = I.trim()
      .replace(Selectors.RegExps.splitter, function (Z, Y, X) {
        B.push(Y);
        return ':)' + X;
      })
      .split(':)');
  ...
  search: function (A) {
    ...
    Array.each(document.styleSheets, function (E, D) {
      ...
      Array.each(F, function (I, G) {
        ...
        var H = I.selectorText
          ? I.selectorText.replace(/^\w+/, function (J) {
...
Element.Properties.send = {
  set: function (A) {
    ...
    return this.eliminate('send').store(
      'send:options',
      $extend({ data: this, link: 'cancel', method: this.get('method') || 'post', url: this.get('action') }, A)
    );
  },
  get: function (A) {
