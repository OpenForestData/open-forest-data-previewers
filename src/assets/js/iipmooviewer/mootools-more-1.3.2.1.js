...
var Drag = new Class({
  ...
  initialize: function () {
    ...
    this.handles =
      (htype == 'array' || htype == 'collection' ? $$(this.options.handle) : document.id(this.options.handle)) ||
      this.element;
    this.mouse = { now: {}, pos: {} };
  ...
  check: function (event) {
    ...
    var distance = Math.round(
      Math.sqrt(Math.pow(event.page.x - this.mouse.start.x, 2) + Math.pow(event.page.y - this.mouse.start.y, 2))
    );
